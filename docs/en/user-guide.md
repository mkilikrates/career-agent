...
5. **Output** — generate a tailored CV in Markdown, PDF, and Word, plus advisory LinkedIn suggestions. When AI tailoring is enabled, the model now produces a **complete ATS-formatted Markdown CV draft** (not just 5 advisory bullet suggestions). The draft includes a targeted professional summary, experience with adjusted emphasis, skills ordered by relevance, education, and core competencies — all built exclusively from your confirmed evidence. You can **toggle between the AI draft and the deterministic version** to compare them side by side — the AI draft is shown as the primary view by default so you can immediately review the AI's work, with a toggle to switch to the deterministic version for comparison. The draft is advisory: you review and confirm it before it replaces the deterministic version. Skills in the output are grouped by category (Technical, Tools, Domain, Leadership, Communication, Core Competency) and sorted by relevance to your target role. In AI-only mode, clicking "Generate CV" directly calls the AI to produce a tailored draft — no separate step is needed.
