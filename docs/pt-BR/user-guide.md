...
- **Barra lateral** (esquerda) — contém o **Indicador de Fases**, que mostra seu progresso no pipeline de forma visual. Cada fase exibe "Concluído" apenas quando seu artefato definidor está confirmado no seu Repositório de Memória (não baseado apenas na posição), de modo que o status sempre reflete o que você realmente completou — fases pelas quais você apenas navegou sem confirmar não serão marcadas como concluídas. Abaixo do indicador você encontra um link para **Configurações** e um botão **Salvar e Sair**.
...
2. **Mapa de Competências** — o Agente de Carreira monta uma lista das suas competências, cada uma ligada ao local onde encontrou a evidência. Você revisa e confirma. Quando o modo IA está selecionado, a IA pode extrair um perfil estruturado e compatível com ATS a partir dos seus documentos — posições de emprego com empresas, localidades, descrições de cargo, conquistas quantificadas e tecnologias utilizadas; educação com instituições e habilidades adquiridas; um resumo profissional; competências essenciais (qualidades comportamentais e de liderança inferidas a partir de padrões de carreira e conquistas, não apenas palavras-chave literais — ex.: Liderança, Planejamento Estratégico, Colaboração Multifuncional); idiomas falados com níveis de proficiência; hobbies e interesses; causas e trabalho voluntário; e quaisquer categorias adicionais (publicações, patentes, prêmios, etc.). As datas são automaticamente normalizadas para formato ISO e entradas compostas de competências são expandidas em itens individuais (com uma lista de permissões protegendo compostos conhecidos como CI/CD, TCP/IP, OS/2, L2/L3 e outros de divisão incorreta). Um passe de detecção de fragmentos também reassembla entradas que foram acidentalmente divididas em etapas anteriores. Após a extração, um **passe de consolidação por IA** (nos modos Somente IA ou IA assistida) envia os dados mesclados ao modelo para deduplicação contextual — detectando nomes de empresas corrompidos por OCR, variantes de competências com prefixo de fornecedor e duplicatas semânticas que regras baseadas em padrões não conseguem detectar de forma confiável. No modo IA assistida, uma rede de segurança determinística leve é executada em seguida como último recurso. No modo Somente IA, a consolidação da IA é o resultado final — nenhum passe determinístico é aplicado por cima. No modo somente-script, a consolidação determinística completa cuida da deduplicação (colapso de prefixos de fornecedor, correspondência fuzzy de cargos e resolução de sinônimos). O pipeline também detecta relações de termo contido (ex.: "GitHub" vs "GitHub Enterprise"), retenção de compostos com barra e variantes de capitalização. As tecnologias são extraídas como itens atômicos independentes (nunca agrupadas por fornecedor), resultando em um mapa de competências mais limpo. As datas `since` das competências são derivadas da evidência de emprego mais antiga onde a competência aparece, garantindo cronogramas de experiência precisos. Essa extração estruturada é apresentada para sua revisão antes de entrar no mapa de competências, fornecendo contexto mais rico sobre onde e quando cada competência foi utilizada. No modo Somente IA, confirmar a extração constrói automaticamente o mapa de competências — nenhum passo separado de "Gerar" é necessário. Você pode adicionar competências manualmente a qualquer momento e o mapa se atualiza imediatamente.
3. **Descoberta de Funções** — receba funções sugeridas que combinam com suas competências, com as lacunas destacadas. A IA agora também recebe o contexto da sua trajetória de carreira (cargos anteriores, competências essenciais, formação e resumo profissional) para produzir recomendações de funções mais precisas, calibradas ao seu nível de senioridade e arco de experiência.
4. **Treinamento para Entrevista** — pratique respostas STAR. Você pode **digitar**, **enviar um arquivo de áudio** ou **gravar a si mesmo** no navegador. A IA verifica se sua resposta está completa e faz perguntas de acompanhamento — usando apenas as suas próprias palavras. As perguntas de prática agora podem sondar múltiplos comportamentos simultaneamente (ex.: "Liderança e Gestão de Stakeholders"), proporcionando uma preparação mais rica para entrevistas.
5. **Geração de Saída** — gere um currículo personalizado em Markdown, PDF e Word, além de sugestões consultivas do LinkedIn. Quando a personalização por IA está habilitada, o modelo agora produz um **rascunho completo de currículo em Markdown no formato ATS** (não apenas 5 sugestões de bullet consultivas). O rascunho inclui um resumo profissional direcionado, experiência com ênfase ajustada, competências ordenadas por relevância, formação, certificações, prêmios, nacionalidade e competências essenciais — tudo construído exclusivamente a partir das suas evidências confirmadas. Seu nome e dados de contato são incluídos automaticamente para que a IA nunca use espaços reservados genéricos. Você pode **alternar entre o rascunho da IA e a versão determinística** para compará-los lado a lado — o rascunho da IA é exibido como a visualização principal por padrão, para que você possa revisar imediatamente o trabalho da IA, com um botão de alternância para mudar para a versão determinística para comparação. O rascunho é orientativo: você o revisa e confirma antes que ele substitua a versão determinística. As competências na saída são agrupadas por categoria (Técnica, Ferramentas, Domínio, Liderança, Comunicação, Competência Essencial) e ordenadas por relevância para a sua vaga-alvo. A duração das competências reflete a janela real de uso (tempo entre o primeiro uso e a última evidência) em vez do tempo desde o primeiro uso, de modo que competências legadas mostram anos precisos. No modo Somente IA, clicar em 'Gerar CV' chama a IA diretamente para produzir um rascunho personalizado — nenhum passo separado é necessário. O **relatório consultivo do LinkedIn** sugere um título (composto do título da vaga-alvo e das 3 principais competências essenciais confirmadas), bullets de experiência e competências recomendadas filtradas por recência (corte de 10 anos) e limitadas a 50.
